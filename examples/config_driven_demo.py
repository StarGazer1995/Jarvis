...
async def demo_provider_switching(config_path: str | None = None) -> None:
    ...
    try:
        ...
        # 测试每个提供商
        test_message = [LLMMessage(role="user", content="请用一句话介绍人工智能。")]
    ...
    for env_name, config_file in environments:
        ...
        try:
            ...
            if enabled_providers:
                ...
                messages = [
                    LLMMessage(
                        role="user", content=f"这是来自{env_name}环境的测试消息。"
                    )
                ]
                response = await client.chat_completion(messages)
...
async def demo_performance_comparison(config_path: str | None = None) -> None:
    ...
    try:
        ...
        if results:
            ...
            print(
                f"{'提供商':<15} {'模型':<20} {'响应时间':<10} {'总时间':<10} {'内容长度':<10}"
            )
            print("-" * 75)

            for result in sorted(results, key=lambda x: x["response_time"]):
                print(
                    f"{result['provider']:<15} {result['model']:<20} "
...
async def interactive_demo(config_path: str | None = None) -> None:
    ...
    try:
        ...
        while True:
            try:
                choice = input(
                    f"\n请选择提供商 (1-{len(enabled_providers)}, 或输入 'q' 退出): "
                ).strip()
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"   (模型: {response.model}, 响应时间: {response.response_time:.2f}秒)"
                )
