...
from core.security_manager import (
    SecurityLevel, PermissionType, ValidationResult,
    SecurityPolicy, SecurityContext, ValidationRequest, ValidationResponse,
    RateLimiter, InputValidator, AuditLogger, ARKSecurityManager
)
...
async def demonstrate_security_policy_enforcement():
    ...
    for i, test_op in enumerate(test_operations, 1):
        ...
        context = SecurityContext(
            ...
            permissions=[PermissionType.FILE_READ, PermissionType.NETWORK_ACCESS, PermissionType.SYSTEM_ACCESS],
            security_level=SecurityLevel.MEDIUM,
...
async def demonstrate_rate_limiting():
    ...
    for scenario in test_scenarios:
        ...
        for attempt in range(1, scenario['attempts'] + 1):
            # Check rate limit
            is_allowed = await rate_limiter.check_limit(f"user_demo:{scenario['operation']}")
        ...
        remaining = await rate_limiter.get_remaining(f"user_demo:{scenario['operation']}")
...
async def demonstrate_input_validation():
    ...
    print(f"   Accuracy: {accuracy:.1%} ({correct_predictions}/{len(validation_test_cases)})")
...
async def demonstrate_audit_logging():
    ...
    security_events = [
        ...
        },
        {
            ...
            "user_id": "user_002",
            "action": "login_failed",
            "details": {"ip_address": "10.0.0.50", "attempts": 3, "reason": "invalid_password"},
            "risk_level": "medium"
        },
        {
            ...
            "details": {"resource": "/api/users", "permission": "read", "method": "GET"},
            "risk_level": "low"
        },
        {
            "event_type": "authorization",
            ...
            "details": {"resource": "/admin/config", "permission": "write", "reason": "insufficient_privileges"},
            "risk_level": "medium"
        },
        {
        ...
        },
        {
            ...
            "user_id": "user_001",
            "action": "api_request",
            "details": {"url": "https://api.example.com/data", "method": "GET", "status_code": 200},
            "risk_level": "low"
        },
        {
            "event_type": "network_activity",
            ...
            "details": {"url": "http://malicious.site.com", "blocked": True, "reason": "blacklisted_domain"},
            "risk_level": "high"
        }
    ]
    ...
    high_risk_events = [e for e in security_events if e['risk_level'] in ['high', 'critical']]
...
async def demonstrate_comprehensive_security_workflow():
    ...
    complex_operations = [
        {
            ...
            "steps": [
                ...
                {"action": "read_database", "target": "SELECT * FROM users WHERE active = 1"},
        ...
        },
        {
            ...
            "steps": [
                ...
                {"action": "upload_backup", "target": "https://backup.example.com/upload"}
            ],
    ...
    for i, operation in enumerate(complex_operations, 1):
        ...
        print(f"   Summary:")
        print(f"     - Success rate: {success_rate:.1%} ({successful_steps}/{len(operation['steps'])})")
        print(f"     - Average risk score: {avg_risk_score:.3f}")
        print(f"     - Operation status: {'✅ Completed' if success_rate == 1.0 else '⚠️ Partially completed' if success_rate > 0 else '❌ Failed'}")
