...
async def main():
    ...
    async def writer_logic(state: MultiAgentState) -> dict[str, Any]:
        ...
        research_summary = structured_data.get(
            "research_summary", "No research data found."
        )
    ...
    async def qa_logic(state: MultiAgentState) -> dict[str, Any]:
        ...
        async def generator(p: str) -> str:
            res = await llm_manager.generate_response(
        ...
        saved_path = loop.save_result(
            final_result, directory="reports", prefix="multi_agent_summary", task=task
        )
    ...
    agents = [
        AgentSpec(
            name="Researcher",
            description="Use this for finding information, data, or facts.",
            node=research_node,
        ),
        AgentSpec(
            name="Writer",
            description="Use this to write drafts, summaries, or content based on research.",
            node=writer_node,
        ),
        AgentSpec(
            name="QualityAssurance",
            description="Use this to review, refine, and polish text. ALWAYS use this before finishing.",
            node=qa_node,
        ),
    ]
            ...
            if key == "supervisor":
                ...
                if msgs:
                    ...
                    if "structured_data" in value and value["structured_data"]:
                        print(
                            f"   (Shared Data Updated: {list(value['structured_data'].keys())})"
                        )
            print("...")
