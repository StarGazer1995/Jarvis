...
async def demonstrate_memory_management():
    ...
    for i in range(15):  # More than max_turns
        turn = ConversationTurn(
            ...
            entities=[
                Entity(type="number", value=str(i+1), confidence=0.9)
            ],
            context_data={
                "test_data": f"Large context data for turn {i+1}" * 10,  # Increase memory usage
                "iteration": i+1,
                "memory_test": True
            },
        ...
        # Check memory usage
        stats = await context.get_context_stats()
        print(f"   Turn {i+1:2d}: {stats['total_turns']} turns, "
              f"{stats['memory_usage_mb']:.2f}MB used")
        
        # Show when cleanup occurs
        if stats['total_turns'] < i + 1:
            print(f"   🧹 Memory cleanup occurred! Turns reduced to {stats['total_turns']}")
...
async def demonstrate_context_persistence():
    ...
    try:
        ...
        print(f"   Session ID match: {original_context.session_id == loaded_context.session_id}")
        ...
        if turns_match:
            for orig, loaded in zip(original_turns, loaded_turns):
                if orig.turn_id != loaded.turn_id or orig.user_input != loaded.user_input:
                    turns_match = False
                    break
        
        print(f"   Turn data integrity: {'✅ Preserved' if turns_match else '❌ Corrupted'}")
...
async def demonstrate_multi_session_context():
    ...
    session_data = [
        {
            ...
            "turns": [
                ...
                ("What tools are best for team collaboration?", IntentType.INFORMATION_REQUEST),
        ...
        },
        {
            ...
            "turns": [
                ("I want to learn about neural networks", IntentType.INFORMATION_REQUEST),
    ...
    for session_info in session_data:
        ...
        for i, (input_text, intent) in enumerate(session_info["turns"], 1):
            turn = ConversationTurn(
                ...
                user_input=input_text,
                intent=intent,
                entities=[Entity(type="topic", value=session_info["topic"], confidence=0.9)],
        ...
        print(f"✅ Created session: {session_info['id']} ({len(session_info['turns'])} turns)")
    ...
    total_turns = sum(len(await ctx.get_recent_turns(count=100)) for ctx in sessions.values())
    ...
    print(f"   - Most active session: {most_active} ({session_activity[most_active]} turns)")
