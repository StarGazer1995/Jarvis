...
class JarvisAgent:
    ...
    async def process_message(self, message: str, user_id: Optional[str] = None) -> str:
        ...
        try:
            self.logger.debug(f"Processing message from user {user_id}: '{message[:50]}...'")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health_status["components"]["agent"] = {
            "status": "healthy" if self.is_running and self.is_initialized else "unhealthy",
            "is_running": self.is_running,
            "is_initialized": self.is_initialized
        }
    ...
    def __repr__(self) -> str:
        ...
        return (f"JarvisAgent(name='{self.config.name}', version='{self.config.version}', "
