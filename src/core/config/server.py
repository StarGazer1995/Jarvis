...
class MCPServerConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "MCPServerConfig":
        ...
        if isinstance(security_level, str):
            security_level = (
                SecurityLevel(security_level) if SecurityLevel else security_level
            )
...
class EnvironmentConfigProvider(ServerConfigProvider):
    ...
    async def load_configs(self) -> List[MCPServerConfig]:
        ...
        try:
            ...
            env_vars = {
                k: v for k, v in os.environ.items() if k.startswith(self.prefix)
            }
            ...
            for server_name, data in server_data.items():
                try:
                    ...
                    config_data = {
                        ...
                        if SecurityLevel
                        else "medium",  # Default security
                        "timeout": int(data.get("timeout", 30)),
                        "max_retries": int(data.get("max_retries", 3)),
                        "description": data.get(
                            "description", f"Environment config for {server_name}"
                        ),
                    }
            ...
            self.logger.info(
                f"Loaded {len(configs)} server configurations from environment"
            )
    ...
    async def delete_config(self, server_name: str) -> bool:
        ...
        raise NotImplementedError(
            "Delete operation not supported for EnvironmentConfigProvider"
        )
...
class ARKServerConfigManager:
    ...
    def add_provider(self, provider: ServerConfigProvider) -> None:
        ...
        if provider not in self.providers:
            ...
            self.logger.info(
                f"Added configuration provider: {provider.__class__.__name__}"
            )
            ...
            try:
                ...
                for config in configs:
                    if config.is_valid():
                        ...
                        _, errors = config.validate()
                        self.logger.warning(
                            f"Invalid config from provider {config.name}: {errors}"
                        )
            except Exception as e:
                self.logger.error(
                    f"Failed to load from provider {provider.__class__.__name__}: {e}"
                )
    ...
    async def save_to_providers(self) -> bool:
        ...
        for provider in self.providers:
            try:
                ...
                self.logger.info(
                    f"Saved configs to provider: {provider.__class__.__name__}"
                )
            except Exception as e:
                self.logger.error(
                    f"Failed to save to provider {provider.__class__.__name__}: {e}"
                )
                success = False
    ...
    def update_server_status(self, server_name: str, status: ServerStatus) -> bool:
        ...
        self.logger.info(
            f"Server {server_name} status changed: {old_status.value} -> {status.value}"
        )
    ...
    async def _check_server_health(self, config: MCPServerConfig) -> None:
        ...
        try:
            ...
            if config.status == ServerStatus.CONNECTED:
                ...
                self.logger.debug(
                    f"Server {config.name} health check: {config.status.value}"
                )
    ...
    def list_configs(
        ...
        if config_type is not None:
            configs = [
                config for config in configs if config.config_type == config_type
            ]

        if security_level is not None:
            configs = [
                config for config in configs if config.security_level == security_level
            ]
            ...
            for config in enabled_configs:
                try:
                    ...
                    async with session.get(
                        health_url, timeout=config.timeout
                    ) as response:
                    ...
                    health_status[config.name] = {"healthy": False, "error": str(e)}
