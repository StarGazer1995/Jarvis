...
from .server_config import SimpleMCPServerConfig, ServerStatus
...
class ARKMCPClient:
        ...
        for tool_key, tool_info in all_tools.items():
            ...
            if tool_key in self._builtin_tools:
                ...
                tool_entry = {
                    ...
                    "inputSchema": tool_info.get("parameters", tool_info.get("inputSchema", {})),
                    "server": server_name,
                    "full_name": tool_key
                }
    ...
    async def _connect_to_server(self, server_name_or_config, server_config=None) -> bool:
        """
    ...
    async def _connect_to_server_internal(self, server_name: str, server_config: SimpleMCPServerConfig) -> None:
        """
        连接到指定的MCP服务器（内部实现）
