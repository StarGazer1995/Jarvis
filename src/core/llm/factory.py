...
class LLMProviderFactory:
    ...
    def load_config(
        ...
        if reload or self._config is None:
            self._config = load_llm_config(
                environment=environment, config_path=self.config_path, reload=reload
            )
    ...
    def _create_provider_client(
        ...
        if client_class is None:
            raise ValueError(
                f"提供商类型 '{provider_type}' (名称: '{provider_name}') 未注册"
            )
        ...
        try:
            ...
            # 转换为LLMConfig对象
            from .types import LLMConfig as ClientLLMConfig
    ...
    def list_enabled_providers(self, environment: Optional[str] = None) -> List[str]:
        ...
        return [
            ...
            and self.registry.is_provider_registered(provider_config.type or name)
        ]
    ...
    def validate_provider_config(
        ...
        try:
            ...
            if not self.registry.is_provider_registered(provider_type):
                logger.error(
                    f"提供商类型 '{provider_type}' (用于 '{provider_name}') 未注册"
                )
                return False
