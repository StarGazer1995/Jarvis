...
class TestARKMCPClient:
    ...
    async def test_start_client(self, mcp_client):
        ...
        with patch.object(
            mcp_client, "_discover_tools", new_callable=AsyncMock
        ) as mock_discover:
    ...
    async def test_stop_client(self, mcp_client):
        ...
        # Stop client
        with patch.object(
            mcp_client, "_disconnect_all_servers", new_callable=AsyncMock
        ) as mock_disconnect:
    ...
    async def test_discover_tools(self, mcp_client, mock_config):
        ...
        with patch.object(
            mcp_client, "discover_tools", new_callable=AsyncMock
        ) as mock_discover:
    ...
    async def test_execute_tool_success(self, mcp_client):
        ...
        # Mock tool execution
        with patch.object(
            mcp_client, "_execute_server_tool", new_callable=AsyncMock
        ) as mock_execute:
    ...
    async def test_connect_to_server_success(self, mcp_client, mock_config):
        ...
        with patch.object(
            mcp_client, "_connect_to_server_internal", new_callable=AsyncMock
        ) as mock_connect:
...
class TestMCPClientIntegration:
    ...
    async def test_full_workflow(self):
        ...
        config = SimpleMCPServerConfig(name="test_server", command=["echo", "hello"])
        ...
        try:
            ...
            with (
                patch.object(
                    client, "_connect_to_servers", new_callable=AsyncMock
                ) as mock_connect,
                patch.object(
            ...
            with patch.object(
                client, "_disconnect_all_servers", new_callable=AsyncMock
            ) as mock_disconnect:
                await client.stop()
...
class TestARKMCPClientIntegration:
    ...
    async def test_call_tool_with_exception(self, client):
        ...
        with patch.object(
            client, "_call_builtin_tool", side_effect=Exception("Test error")
        ):
            result = await client.call_tool("echo", {"message": "test"})
            assert result["success"] is False
...
class TestARKMCPClientErrorHandling:
    ...
    async def test_connect_to_servers_error(self, client):
        ...
        client.servers["test_server"] = SimpleMCPServerConfig(
            name="test_server", command=["echo", "test"], args=[], enabled=True
        )
    ...
    def test_get_server_status_disconnected(self, client):
        ...
        config = SimpleMCPServerConfig(
            name="test_server", command=["echo", "test"], args=[]
        )
...
class TestARKMCPClientAdvancedFeatures:
    ...
    async def test_connect_to_server_internal_error_handling(self, client):
        ...
        config = SimpleMCPServerConfig(name="test_server", command=["invalid_command"])
        client.servers["test_server"] = config
    ...
    async def test_close_method_coverage(self, client):
        ...
        client.sessions = {"server1": mock_session1, "server2": mock_session2}
        ...
        with patch.object(
            client, "disconnect_server", side_effect=mock_disconnect
        ) as mock_disc:
            # Test close
            await client.close()
    ...
    async def test_list_tools_with_mcp_server_tools(self, client):
        ...
        client.available_tools = {"test_server:mcp_tool": mock_tool}
    ...
    async def test_call_tool_simple_name_matching(self, client):
        ...
        client.available_tools = {"test_server:simple_tool": mock_tool}
        client.sessions = {"test_server": mock_session}
    ...
    async def test_call_tool_successful_execution(self, client):
        ...
        # Set available_tools and sessions
        client.available_tools = {"test_server:test_tool": mock_tool}
        client.sessions = {"test_server": mock_session}
    ...
    async def test_demo_method_coverage(self, client):
        ...
        config = SimpleMCPServerConfig(
            name="test_server", command=["echo", "test"], description="Test server"
        )
        client.servers = {"test_server": config}
        client.available_tools = {
        ...
        with patch.object(
            client, "get_server_status", return_value=ServerStatus.CONNECTED
        ):
    ...
    async def test_close_method_exception_handling(self, client):
        ...
        with patch.object(
            client, "_disconnect_all_servers", side_effect=Exception("Disconnect error")
        ):
    ...
    async def test_connect_to_server_success_path(self, client):
        ...
        with (
            patch.object(client, "add_server", return_value=True) as mock_add,
            patch.object(
                client, "_connect_to_server", return_value=True
            ) as mock_connect,
        ):
    ...
    async def test_connect_to_server_internal_success_path(self, client):
        ...
        server_name = "test_server"
        server_config = SimpleMCPServerConfig(
            name=server_name, command="python", args=["-m", "test"]
        )
        ...
        with (
            patch("src.core.mcp.client.stdio_client", return_value=mock_context),
    ...
    async def test_disconnect_all_servers_success(self, client):
        ...
        # Mock disconnect_server to succeed
        with patch.object(
            client, "disconnect_server", new_callable=AsyncMock, return_value=True
        ) as mock_disconnect:
    ...
    async def test_close_method_with_disconnect_exception(self, client):
        ...
        with patch.object(
            client, "disconnect_server", side_effect=Exception("Disconnect failed")
        ):
    ...
    async def test_remove_server_with_delete_config(self, client):
        ...
        server_name = "test_server"
        server_config = SimpleMCPServerConfig(
            name=server_name, command="test_command", args=[]
        )
