                ...
                for server_config in mcp_servers:
                    ...
                    if success:
                        self.ark_logger.info(
                            f"SimpleARK: Connected to '{server_config.name}'"
                        )
                    else:
                        self.ark_logger.warning(
                            f"SimpleARK: Failed to connect to '{server_config.name}'"
                        )
            ...
            for server_name in self.mcp_client.sessions.keys():
                try:
                    ...
                    for tool in server_tools:
                        tool_name = tool.get(
                            "name", f"unknown_{len(self.available_tools)}"
                        )
                        self.available_tools[tool_name] = tool
                except Exception as e:
                    self.ark_logger.warning(
                        f"Error discovering tools from {server_name}: {e}"
                    )
