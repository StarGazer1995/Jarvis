...
class ToolMetadata:
    ...
    parameters: Dict[str, Any] = field(default_factory=dict)  # For backward compatibility
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ToolMetadata':
        ...
        return cls(
            ...
            created_at=datetime.fromisoformat(data.get("created_at")) if isinstance(data.get("created_at"), str) else datetime.fromtimestamp(data.get("created_at", time.time())),
            updated_at=datetime.fromisoformat(data.get("updated_at")) if isinstance(data.get("updated_at"), str) else datetime.fromtimestamp(data.get("updated_at", time.time()))
        )
...
class ToolFilter:
    ...
    def matches(self, tool: ToolMetadata) -> bool:
        ...
        # Name pattern filter
        if self.name_pattern and not fnmatch.fnmatch(tool.name.lower(), self.name_pattern.lower()):
            return False
        
        # Description pattern filter
        if (self.description_pattern and 
            self.description_pattern.lower() not in tool.description.lower()):
            return False
...
class ToolDiscoveryProvider(ABC):
    ...
    @abstractmethod
    async def discover_tools(self, tool_filter: Optional['ToolFilter'] = None) -> List[ToolMetadata]:
        """
...
class MCPToolDiscoveryProvider(ToolDiscoveryProvider):
        ...
        try:
            ...
            for tool_info in mcp_tools:
                try:
                    ...
                    self.logger.warning(f"Failed to create metadata for tool {tool_info.get('name', 'unknown')}: {e}")
    ...
    def _create_tool_metadata(self, tool_name: str, tool_info: Dict[str, Any]) -> ToolMetadata:
        ...
        tool_parameters = parameters_schema.get('properties', {}) if parameters_schema else {}
    ...
    def _generate_tags(self, tool_name: str, description: str) -> List[str]:
        ...
        desc_words = [word.lower().strip('.,!?;:') for word in description.split() 
...
class FileToolDiscoveryProvider(ToolDiscoveryProvider):
    ...
    async def discover_tools(self, tool_filter: Optional['ToolFilter'] = None) -> List[ToolMetadata]:
        ...
        try:
            ...
            self.logger.info(f"Discovered {len(tools)} tools from file {self.file_path}")
            return tools
...
class ARKToolRegistry:
    ...
    def remove_discovery_provider(self, provider: Union[ToolDiscoveryProvider, str]) -> None:
        """
        ...
        if isinstance(provider, str):
            ...
            for p in list(self.discovery_providers):
                if hasattr(p, 'name') and p.name == provider:
                    ...
                    self.logger.info(f"Removed discovery provider: {p.__class__.__name__}")
                    return
        else:
            # Remove by object
            if provider in self.discovery_providers:
                self.discovery_providers.remove(provider)
                self.logger.info(f"Removed discovery provider: {provider.__class__.__name__}")
    
    async def discover_tools(self, tool_filter: Optional[ToolFilter] = None, force_refresh: bool = False) -> List[ToolMetadata]:
        """
        ...
        for provider in self.discovery_providers:
            try:
                ...
                self.logger.info(f"Provider {provider.__class__.__name__} discovered {len(tools)} tools")
                
            except Exception as e:
                self.logger.error(f"Discovery failed for provider {provider.__class__.__name__}: {e}")
    ...
    def get_tools_by_category(self, category: Union[ToolCategory, str]) -> List[ToolMetadata]:
        """
    ...
    def get_recent_tools(self, limit: int = 10) -> List[ToolMetadata]:
        ...
        """
        recent_tools = [tool for tool in self.tools.values() if tool.last_used is not None]
        sorted_tools = sorted(recent_tools, key=lambda t: t.last_used or 0, reverse=True)
        return sorted_tools[:limit]
    
    def record_tool_execution(self, tool_name: str, execution_time: float, success: bool, error: Optional[str] = None) -> None:
        """
    ...
    def record_execution(self, tool_name: str, success: bool, execution_time: float, error: Optional[str] = None) -> None:
