...
class TestJarvisConfig:
    ...
    def test_config_creation_full(self):
        ...
        mock_server = SimpleMCPServerConfig(
    ...
    def test_config_to_dict(self):
        ...
        config = JarvisConfig(
            name="TestJarvis", log_level="DEBUG", max_conversation_history=50
        )
...
class TestJarvisAgent:
    ...
    async def test_process_message(self, agent):
        ...
        with patch.object(
            agent.ark_engine, "process_input", new_callable=AsyncMock
        ) as mock_process:
    ...
    async def test_start_conversation(self, agent):
        ...
        with patch.object(
            agent.ark_engine.context_manager, "reset_session", return_value=None
        ):
    ...
    async def test_end_conversation(self, agent):
        ...
        with patch.object(
            ...
            "get_session_stats",
            return_value={"turn_count": 5, "duration_minutes": 10.5},
        ):
    ...
    async def test_get_conversation_export(self, agent):
        ...
        with patch.object(
            agent.ark_engine.context_manager,
            "export_conversation",
            return_value='{"conversation": [{"user": "Hello", "agent": "Hi there!"}]}',
        ):
            # Test JSON export
    ...
    async def test_context_manager(self, agent):
        ...
        with (
            patch.object(agent, "initialize", return_value=True),
...
class TestJarvisAgentAdvanced:
    ...
    async def test_initialization_with_servers(self, agent_with_servers):
        ...
        with (
            patch.object(
                agent_with_servers.ark_engine.mcp_client,
                "connect_to_server",
                return_value=True,
            ) as mock_connect,
    ...
    async def test_process_message_with_tool_usage(self):
        ...
        with (
            patch.object(agent.ark_engine, "process_input", return_value="response"),
    ...
    async def test_startup_callbacks_execution(self):
        ...
        with (
            patch.object(agent.ark_engine, "initialize", return_value=True),
    ...
    async def test_shutdown_callbacks_execution(self):
        ...
        with (
            patch.object(agent.ark_engine.mcp_client, "stop"),
