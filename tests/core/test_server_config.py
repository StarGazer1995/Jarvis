...
from src.core.server_config import (
    ServerType, ServerStatus, AuthType, ConfigType,
    MCPServerConfig, SimpleMCPServerConfig,
    ServerCredentials, ServerHealthCheck, ServerLimits, ServerMetrics,
...
class TestFileConfigProvider:
    ...
    @pytest.mark.asyncio
    async def test_file_provider_save_config_single_file_mode_update_existing(self, temp_config_file):
    ...
    @pytest.mark.asyncio
    async def test_file_provider_save_config_single_file_mode_yaml(self, temp_config_file):
    ...
    @pytest.mark.asyncio
    async def test_file_provider_delete_config_single_file_mode_success(self, temp_config_file):
    ...
    @pytest.mark.asyncio
    async def test_file_provider_delete_config_single_file_mode_not_found(self, temp_config_file):
    ...
    @pytest.mark.asyncio
    async def test_file_provider_delete_config_single_file_mode_yaml(self, temp_config_file):
    ...
    @pytest.mark.asyncio
    async def test_file_provider_delete_config_directory_mode_json(self, temp_config_dir):
    ...
    @pytest.mark.asyncio
    async def test_file_provider_delete_config_directory_mode_yaml(self, temp_config_dir):
    ...
    @pytest.mark.asyncio
    async def test_file_provider_delete_config_directory_mode_not_found(self, temp_config_dir):
...
class TestARKServerConfigManagerAdvanced:
    ...
    async def test_start_and_stop(self, manager):
        ...
        with unittest.mock.patch.object(manager, '_health_check_loop') as mock_health_loop:
    ...
    async def test_load_configs_with_invalid_config(self, manager):
        ...
        manager.config_provider.load_configs.return_value = [valid_config, invalid_config]
    ...
    def test_update_config_exception(self, manager):
        ...
        try:
            ...
            with unittest.mock.patch('src.core.server_config.time.time', side_effect=Exception("Time error")):
                result = manager.update_config("test-server", command="new-command")
                assert result is False
        finally:
            # Re-enable logging
    ...
    def test_validate_config_exception(self, manager):
        ...
        with unittest.mock.patch.object(config, 'validate', side_effect=Exception("Validation error")):
    ...
    async def test_load_from_providers(self, manager):
        ...
        provider2.load_configs = unittest.mock.AsyncMock(return_value=[config2, invalid_config])
    ...
    async def test_load_from_providers_with_exception(self, manager):
        ...
        provider1.load_configs = unittest.mock.AsyncMock(return_value=[config1])
        provider2.load_configs = unittest.mock.AsyncMock(side_effect=Exception("Provider error"))
    ...
    async def test_save_to_providers_with_exception(self, manager):
        ...
        provider = unittest.mock.Mock(spec=FileConfigProvider)
        provider.save_configs = unittest.mock.AsyncMock(side_effect=Exception("Save error"))
    ...
    async def test_health_check_loop_exception(self, manager):
        ...
        with unittest.mock.patch.object(manager, '_check_server_health', side_effect=Exception("Health check error")):
            manager._running = True
            task = asyncio.create_task(manager._health_check_loop())
    ...
    def test_get_security_levels_without_security_level(self, manager):
        ...
        with unittest.mock.patch('src.core.server_config.SecurityLevel', None):
    ...
    def test_get_security_levels_with_configs(self, manager):
        ...
        from src.core.server_config import SecurityLevel
...
class TestEnvironmentConfigProviderErrorHandling:
    ...
    async def test_env_provider_load_configs_with_invalid_json(self):
        ...
        with patch.dict(os.environ, {
            'MCP_SERVER_CONFIG_test_server': 'invalid json {'
        }):
            provider = EnvironmentConfigProvider()
            configs = await provider.load_configs()
...
class TestMCPServerConfigUpdateMethod:
    ...
    def test_update_invalid_attribute(self):
        ...
        with pytest.raises(ValueError, match="Invalid configuration parameter: invalid_attr"):
