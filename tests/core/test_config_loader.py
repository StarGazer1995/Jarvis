...
from src.core.config_loader import (
    ConfigLoader, RetryConfig, TimeoutConfig, ModelConfig, 
    ProviderConfig, GlobalConfig, FeatureConfig, LLMConfig,
    load_llm_config, get_config_path
)
...
class TestConfigLoader:
    ...
    def test_validate_config_valid(self, sample_config_data):
        """测试有效配置验证"""
        from src.core.config_loader import Environment
...
class TestConfigHelperFunctions:
    ...
    @patch('src.core.config_loader.ConfigLoader.load_config')
...
class TestConfigLoaderEdgeCases:
    ...
    def test_config_with_nested_env_vars(self):
        """测试嵌套环境变量"""
        with patch.dict(os.environ, {"BASE_URL": "https://api.example.com", "VERSION": "v1"}):
            loader = ConfigLoader("dummy.yaml")
    ...
    def test_config_with_special_characters(self):
        ...
        assert config.providers["test"].api_key == "key_with_!@#$%^&*()_+-={}[]|\\:;\"'<>?,./"
