...
class TestMCPToolDiscoveryProvider:
    ...
    def test_determine_category_communication_tools(self):
        ...
        # Test email tools
        assert provider._determine_category("send_email", "Send an email message") == ToolCategory.COMMUNICATION
        assert provider._determine_category("email_tool", "Email functionality") == ToolCategory.COMMUNICATION
        
        # Test messaging tools
        assert provider._determine_category("send_message", "Send a message") == ToolCategory.COMMUNICATION
        assert provider._determine_category("slack_notify", "Send Slack notification") == ToolCategory.COMMUNICATION
        
        # Test notification tools
        assert provider._determine_category("notify_user", "Notify the user") == ToolCategory.COMMUNICATION
        assert provider._determine_category("push_notification", "Send push notification") == ToolCategory.COMMUNICATION
    ...
    def test_determine_category_data_analysis_tools(self):
        ...
        # Test analysis tools
        assert provider._determine_category("analyze_data", "Analyze dataset") == ToolCategory.GENERAL
        assert provider._determine_category("data_processor", "Process data") == ToolCategory.GENERAL
        
        # Test chart/graph tools
        assert provider._determine_category("create_chart", "Create a chart") == ToolCategory.GENERAL
        assert provider._determine_category("plot_graph", "Plot a graph") == ToolCategory.GENERAL
        
        # Test statistics tools
        assert provider._determine_category("calculate_stats", "Calculate statistics") == ToolCategory.CALCULATION
        assert provider._determine_category("statistical_analysis", "Statistical analysis") == ToolCategory.CALCULATION
    ...
    def test_determine_category_file_operations_tools(self):
        ...
        # Test file tools
        assert provider._determine_category("read_file", "Read a file") == ToolCategory.FILE_SYSTEM
        assert provider._determine_category("write_file", "Write to file") == ToolCategory.FILE_SYSTEM
        assert provider._determine_category("file_manager", "Manage files") == ToolCategory.FILE_SYSTEM
        
        # Test directory tools
        assert provider._determine_category("list_directory", "List directory contents") == ToolCategory.FILE_SYSTEM
        assert provider._determine_category("create_folder", "Create a folder") == ToolCategory.FILE_SYSTEM
    ...
    def test_determine_category_web_services_tools(self):
        ...
        # Test HTTP tools
        assert provider._determine_category("http_request", "Make HTTP request") == ToolCategory.GENERAL
        assert provider._determine_category("api_call", "Call an API") == ToolCategory.GENERAL
        
        # Test web scraping tools
        assert provider._determine_category("scrape_website", "Scrape web content") == ToolCategory.GENERAL
        assert provider._determine_category("web_crawler", "Crawl websites") == ToolCategory.GENERAL
    ...
    def test_determine_category_system_tools(self):
        ...
        assert provider._determine_category("system_monitor", "Monitor system") == ToolCategory.GENERAL
    ...
    def test_determine_category_development_tools(self):
        ...
        # Test code tools
        assert provider._determine_category("code_formatter", "Format code") == ToolCategory.GENERAL
        assert provider._determine_category("lint_code", "Lint source code") == ToolCategory.GENERAL
        
        # Test git tools
        assert provider._determine_category("git_commit", "Commit to git") == ToolCategory.GENERAL
        assert provider._determine_category("version_control", "Version control operations") == ToolCategory.GENERAL
        
        # Test build tools
        assert provider._determine_category("build_project", "Build the project") == ToolCategory.GENERAL
        assert provider._determine_category("compile_code", "Compile source code") == ToolCategory.GENERAL
    ...
    def test_determine_category_productivity_tools(self):
        ...
        assert provider._determine_category("create_task", "Create a task") == ToolCategory.PRODUCTIVITY
        assert provider._determine_category("todo_manager", "Manage todo items") == ToolCategory.PRODUCTIVITY
        
        # Test calendar tools
        assert provider._determine_category("schedule_meeting", "Schedule a meeting") == ToolCategory.PRODUCTIVITY
        assert provider._determine_category("calendar_event", "Create calendar event") == ToolCategory.PRODUCTIVITY
        
        # Test note tools
        assert provider._determine_category("take_notes", "Take notes") == ToolCategory.PRODUCTIVITY
        assert provider._determine_category("note_manager", "Manage notes") == ToolCategory.PRODUCTIVITY
    ...
    def test_determine_category_entertainment_tools(self):
        ...
        # Test game tools
        assert provider._determine_category("play_game", "Play a game") == ToolCategory.ENTERTAINMENT
        assert provider._determine_category("game_engine", "Game engine functionality") == ToolCategory.ENTERTAINMENT
        
        # Test music tools
        assert provider._determine_category("play_music", "Play music") == ToolCategory.ENTERTAINMENT
        assert provider._determine_category("music_player", "Music player controls") == ToolCategory.ENTERTAINMENT
        
        # Test video tools
        assert provider._determine_category("play_video", "Play video content") == ToolCategory.ENTERTAINMENT
        assert provider._determine_category("video_player", "Video player controls") == ToolCategory.ENTERTAINMENT
    ...
    def test_determine_category_weather_tools(self):
        ...
        # Test weather tools
        assert provider._determine_category("get_weather", "Get weather information") == ToolCategory.WEATHER
        assert provider._determine_category("weather_forecast", "Weather forecast") == ToolCategory.WEATHER
        assert provider._determine_category("climate_data", "Climate data analysis") == ToolCategory.WEATHER
        
        # Test temperature tools
        assert provider._determine_category("temperature_check", "Check temperature") == ToolCategory.WEATHER
        assert provider._determine_category("humidity_sensor", "Read humidity") == ToolCategory.WEATHER
    ...
    def test_determine_category_time_tools(self):
        ...
        # Test time tools
        assert provider._determine_category("get_time", "Get current time") == ToolCategory.TIME
        assert provider._determine_category("time_converter", "Convert time zones") == ToolCategory.TIME
        assert provider._determine_category("timestamp_tool", "Work with timestamps") == ToolCategory.TIME
        
        # Test date tools
        assert provider._determine_category("date_calculator", "Calculate dates") == ToolCategory.TIME
        assert provider._determine_category("calendar_tool", "Calendar operations") == ToolCategory.TIME
    ...
    def test_determine_category_search_tools(self):
        ...
        # Test search tools
        assert provider._determine_category("search_engine", "Search the web") == ToolCategory.SEARCH
        assert provider._determine_category("find_information", "Find information") == ToolCategory.SEARCH
        assert provider._determine_category("lookup_tool", "Look up data") == ToolCategory.SEARCH
        
        # Test database search
        assert provider._determine_category("database_search", "Search database") == ToolCategory.SEARCH
        assert provider._determine_category("query_data", "Query data sources") == ToolCategory.SEARCH
    ...
    def test_determine_category_calculator_tools(self):
        ...
        assert provider._determine_category("calculator", "Perform calculations") == ToolCategory.CALCULATOR
        assert provider._determine_category("math_solver", "Solve math problems") == ToolCategory.CALCULATOR
        assert provider._determine_category("compute_result", "Compute mathematical result") == ToolCategory.CALCULATOR
        
        # Test conversion tools
        assert provider._determine_category("unit_converter", "Convert units") == ToolCategory.CALCULATOR
        assert provider._determine_category("currency_converter", "Convert currency") == ToolCategory.CALCULATOR
    ...
    def test_determine_category_general_fallback(self):
        ...
        # Test unknown tools
        assert provider._determine_category("unknown_tool", "Unknown operation") == ToolCategory.GENERAL
        assert provider._determine_category("mystery_tool", "Mystery operation") == ToolCategory.GENERAL
    ...
    def test_generate_tags_from_name(self):
        ...
        assert "getweatherdata" in tags  # camelCase is converted to lowercase but not split
    ...
    def test_generate_tags_from_description(self):
        ...
        tags = provider._generate_tags("tool", "Get current weather and temperature data")
    ...
    def test_generate_tags_removes_duplicates(self):
        ...
        tags = provider._generate_tags("weather_tool", "weather information for weather data")
...
class TestARKToolRegistry:
    ...
    def test_record_tool_execution_edge_cases(self, registry, sample_tools):
        ...
        registry.record_tool_execution("nonexistent_tool", execution_time=1.0, success=True)
    ...
    async def test_auto_discovery_loop_exception_handling(self, registry):
        ...
        mock_provider = Mock()
        mock_provider.discover_tools = AsyncMock(side_effect=Exception("Discovery failed"))
...
class TestToolRegistryIntegration:
    ...
    async def test_registry_lifecycle_methods(self):
        ...
        assert not hasattr(registry, '_discovery_task') or registry._discovery_task is None
    ...
    async def test_auto_discovery_disabled(self):
        ...
        mock_provider.discover_tools.return_value = [
            ToolMetadata(name="auto_tool", description="Auto discovered", category=ToolCategory.GENERAL)
        ]
        registry.add_discovery_provider(mock_provider)
    ...
    def test_export_tools_json_format(self):
         ...
         # Add some tools
         tool1 = ToolMetadata(name="tool1", description="Tool 1", category=ToolCategory.GENERAL)
         tool2 = ToolMetadata(name="tool2", description="Tool 2", category=ToolCategory.WEATHER)
