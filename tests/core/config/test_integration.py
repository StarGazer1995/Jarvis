...
from src.core.llm.factory import LLMProviderFactory, LLMProviderRegistry, create_llm_client
from src.core.llm.client import BaseLLMClient, LLMMessage, LLMResponse, LLMConfig as ClientLLMConfig
  ...
  secondary:
    ...
    async def test_create_llm_client_function_integration(self, temp_config_file):
        ...
        with patch('src.core.llm.factory.get_llm_factory', return_value=factory):
            ...
            # 测试指定提供商的客户端创建
            client2 = create_llm_client(provider_name="secondary", config_path=temp_config_file)
...
class TestConfigurationErrorScenarios:
    ...
    def test_invalid_default_provider_reference(self):
        ...
        config_data = {
            ...
            "providers": {
                "existing": {"type": "integration_mock", "enabled": True, "default_model": "test", "models": {"test": {}}}
            }
        }
