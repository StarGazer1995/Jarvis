...
async def test_compress_history_triggers_compression(mock_llm_config):
    ...
    # Mock dependencies
    with patch("src.core.context.manager.load_llm_config", return_value=mock_llm_config), \
...
async def test_compress_history_below_threshold(mock_llm_config):
    ...
    with patch("src.core.context.manager.load_llm_config", return_value=mock_llm_config) as mock_load:
