...
class TestARKEngine(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_initialize_success(self):
        ...
        self.mock_mcp_client.discover_tools.return_value = [
        ...
        server_config = SimpleMCPServerConfig(
            name="test_server", command="echo", args=[]
        )
    ...
    async def test_process_input_success_add_task(self):
        ...
        final_state = {
            ...
            "todo_list": [{"id": "1", "description": "New Task", "status": "pending"}],
            "sender": "agent",
        }
    ...
    def test_get_status(self):
        ...
        self.engine.todo_list = [
            Task(id="1", description="t", status=TaskStatus.PENDING)
        ]
    ...
    async def test_legacy_methods(self):
        ...
        with patch(
            "src.core.agent.react.ReActAgent._get_system_prompt",
        ...
        # execute_tool
        with patch(
            "src.core.agent.react.ReActAgent.execute_tool", new_callable=AsyncMock
        ) as mock_exec:
    ...
    async def test_initialize_super_fail(self):
        ...
        with patch(
            "src.core.agent.react.ReActAgent.initialize", new_callable=AsyncMock
        ) as mock_super_init:
    ...
    async def test_initialize_discover_tools_crash(self):
        ...
        with patch.object(
            self.engine, "_discover_tools", side_effect=Exception("Crash")
        ):
    ...
    async def test_shutdown_exceptions(self):
        ...
        self.mock_mcp_client.disconnect_all.side_effect = Exception(
            "MCP Disconnect Fail"
        )  # Trigger catch block

        # Mock context export failure
        self.mock_context_manager.export_conversation.side_effect = Exception(
            "Export fail"
        )
    ...
    async def test_close_alias(self):
        ...
        with patch.object(
            self.engine, "shutdown", new_callable=AsyncMock
        ) as mock_shutdown:
