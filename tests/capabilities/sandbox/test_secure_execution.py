...
class TestSandboxExecution:
    ...
    async def test_sandbox_file_access_policy(self, security_manager, sandbox_context):
        ...
        response = await security_manager.validate_tool_execution(safe_request, "medium")
        assert response.result == ValidationResult.ALLOWED, "Safe file access should be allowed in sandbox"
        ...
        response = await security_manager.validate_tool_execution(unsafe_request, "medium")
        assert response.result == ValidationResult.DENIED, "System file access should be denied in sandbox"
        assert "File path not allowed" in response.message

    @pytest.mark.asyncio
    async def test_sandbox_network_restrictions(self, security_manager, sandbox_context):
        """
        ...
        policy.requires_approval = False  # Disable approval for this test to check filtering only

        response = await security_manager.validate_tool_execution(safe_request, "high")
        assert response.result == ValidationResult.ALLOWED, "Whitelisted domain should be allowed"
        ...
        response = await security_manager.validate_tool_execution(unsafe_request, "high")
        assert response.result == ValidationResult.DENIED, "Unknown domain should be denied in HIGH security mode"
    ...
    async def test_sandbox_rate_limiting(self, security_manager, sandbox_context):
        ...
        assert response.result == ValidationResult.RATE_LIMITED, "Sandbox should enforce rate limits"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Please run via pytest: pytest tests/capabilities/sandbox/test_secure_execution.py")
